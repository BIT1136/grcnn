...
class processor:
    ...
    def crop(self,img,channelAhead=False):
        if len(img.shape)==2:
            return img[self.top_left[0]:self.bottom_right[0], self.top_left[1]:self.bottom_right[1]]
    ...
    def callback(self,data):
        ...
        self.qpub.publish(ros_numpy.msgify(Image,to_u8(q_img).astype(np.uint8),encoding='mono8'))
        self.apub.publish(ros_numpy.msgify(Image,to_u8(ang_img).astype(np.uint8),encoding='mono8'))
        self.wpub.publish(ros_numpy.msgify(Image,to_u8(width_img).astype(np.uint8),encoding='mono8'))
        ...
        self.pub.publish(ros_numpy.msgify(Image, to_u8(q_img).astype(np.uint8), encoding='mono8'))
